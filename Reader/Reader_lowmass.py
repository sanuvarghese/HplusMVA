...
def open_and_process_file(process, systematic, year, channel, outputDir):
    ...
    methodName = "BDT"
    weightfile = os.path.abspath("./TMVA_bdt_planned_weights_lowmass1.xml")  # Ensure correct path
 ...
 // Check if the BDT output is NaN
    ...
    if (std::isnan(Weight)) {
        ...
        continue; // Skip this event
            ...
            if(value < 0.11)continue; //medium wp
...
# Correct call to ProcessLine, ensuring string literals are correctly quoted
    ...
    processEvents("{inputFile}", 
                  "KinFit","{systematic}",  
                  "/afs/cern.ch/work/s/savarghe/private/HplusMVA/CMSSW_13_0_10/src/HplusMVA/Reader/TMVA_bdt_planned_weights_lowmass1.xml",
                  "{outputFileName}" 
                  )
    """
)
