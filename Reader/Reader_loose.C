void Reader_loose(const TString& year, const TString& process, const TString& channel, const TString& mass) {
  ...
  TMVA::Tools::Instance();
    ...
    TString outputDir = Form("/eos/cms/store/group/phys_b2g/savarghe/HplusHistograms/MVA/%s/%s/ExcLoose/", year.Data(), channel.Data());
...
//reader.AddVariable("dPhi_j1b2",'F')
  ...
  for (Long64_t ievt=0; ievt < tree.GetEntries();ievt++) {
      ...
      if(output < -0.5 || output > -0.02)continue; //medium wp
