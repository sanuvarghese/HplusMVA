...
def setTDRStyle():
    ...
    ROOT.gStyle.SetLegendBorderSize(0);  ROOT.gStyle.SetLegendFont(42); ROOT.gStyle.SetLegendTextSize(0.035)
...
def getSummedHistos(file_dict, chName, process):
    ...
    for year, f in file_dict.items():
        dir_name = "shapes_prefit" if process.startswith("WH") else "shapes_fit_s"
...
def getCombinedDataHistogram(file_dict, chName, template_hMC):
    h_total = None
    for year, f in file_dict.items():
        g = f.Get(f"shapes_fit_s/{chName}/data")
        if not g:
            raise ValueError(f"[{year}] Missing data graph: shapes_fit_s/{chName}/data")
...
def example_stack(year_key, category, chLabel):
    ...
    hSig.Scale(0.01 * 5)  # BR scaling

    # ttX
    hTTX = hTTG.Clone("hTTX"); hTTX.Add(hTTH); hTTX.Add(hTTV); hTTX.Add(hTTZ)

    # Color-blind friendly palette (updated names for clarity)
    colors = {
        "VV":    ROOT.TColor.GetColor("#999999"), # Gray
        "Z/gamma+jets": ROOT.TColor.GetColor("#009E73"), # Green
        "W+jets":ROOT.TColor.GetColor("#F0E442"), # Yellow
        "QCD":   ROOT.TColor.GetColor("#E69F00"), # Orange
        "Single t": ROOT.TColor.GetColor("#56B4E9"), # Light Blue
        "ttX":   ROOT.TColor.GetColor("#CC79A7"), # Pink/Purple
        "ttbar+jets": ROOT.TColor.GetColor("#0072B2"), # Blue
    }

    # Stack order (bottom -> top) with human-readable labels and proper color assignment
    draw_list = [
        ("VV",             hVV),
        ("Z/gamma+jets",   hDY),
        ("W+jets",         hWjet),
    ...
    for label, h in draw_list:
        col = colors[label]
        # Set fill color, line color to black, and line width for PDF style
        h.SetFillColor(col)
        h.SetLineColor(ROOT.kBlack) # <--- Set line color to black for PDF style
        h.SetLineWidth(1)
        stack.Add(h)
    ...
    stack.SetMaximum(1000000 * max(stack.GetMaximum(), 1.0))
    ...
    # Legend (two columns)
    leg = TLegend(0.50, 0.46, 0.96, 0.92)
    leg.SetBorderSize(0); leg.SetFillStyle(0); leg.SetTextFont(42); leg.SetNColumns(2)
    
    # Use the human-readable labels defined in draw_list
    leg.AddEntry(hData, "Data", "PE")
    leg.AddEntry(hTT,  "t#bar{t} + jets", "F") # ttbar+jets
    ...
    ratio.GetYaxis().SetNdivisions(504); ratio.SetMinimum(0.89); ratio.SetMaximum(1.11)
    ratio.Draw("EP")
    ...
    if isSaveHisto:
        ...
        out_tag = year_key
        base_filename = f"output/mjj_fit_s_{out_tag}_{category}_mu"

        # 1. Save as PDF (requested)
        c1.SaveAs(base_filename + ".pdf")

        # 2. Save as ROOT file (requested)
