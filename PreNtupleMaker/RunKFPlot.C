...
void RunKFPlot() {
    ...
    // Loop over isMu values (0 and 1)
    for (int isMu = 0; isMu <= 1; ++isMu) {
        // Loop over excllevel values (0 to 3)
        for (int excllevel = 0; excllevel <= 3; ++excllevel) {
            std::cout << "Running KFDDPlot with year = " << year
                      << ", isMu = " << isMu
                      << ", excllevel = " << excllevel << std::endl;
            KFDDPlot(year, isMu, excllevel);
        }
    }
}
