...
tr->SetBranchAddress("isExcTight", &isExcTight);
  ...
  string inpath_hist = Form("/eos/user/i/idas/Output/cms-hcs-run2/CBA_metxycorr/post/%d/all/",year);
...
TFile *fout = new TFile(Form("root_files/%s/post/%d/IsoMET_QCD_%s_%d.root", dir.c_str(), year, signal.c_str(), year), "recreate");
...
int RunAllProcesses() {
    ...
    int year = 2016;
    std::string inpath_tree_base = "/eos/cms/store/group/phys_b2g/savarghe/NewJEC/QCD_tree/post/" + std::to_string(year) + "/WithBDT/";
